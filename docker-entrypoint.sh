...
# Optional emergency switch for 2FA troubleshooting.
if [ -f "$WEBWORK_ROOT/conf/localOverrides.conf" ] && [ "${WEBWORK_DISABLE_2FA:-0}" = "1" ]; then
	if ! grep -q '^\$twoFA{enabled} = 0;' "$WEBWORK_ROOT/conf/localOverrides.conf"; then
		echo '$twoFA{enabled} = 0;' >> "$WEBWORK_ROOT/conf/localOverrides.conf"
		echo "Added 2FA override to localOverrides.conf (WEBWORK_DISABLE_2FA=1)."
	fi
fi

if [ -f "$WEBWORK_ROOT/conf/localOverrides.conf" ] && [ -n "${WEBWORK_SMTP_SENDER:-}" ]; then
	if ! grep -q '^\$twoFA{email_sender}' "$WEBWORK_ROOT/conf/localOverrides.conf"; then
		echo "\$twoFA{email_sender} = '$WEBWORK_SMTP_SENDER';" >> "$WEBWORK_ROOT/conf/localOverrides.conf"
		echo "Configured twoFA email sender from WEBWORK_SMTP_SENDER."
	fi
fi
